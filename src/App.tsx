import { ArrowRight, Check, Code2, Gem, Layers3, Sparkles, Zap } from 'lucide-react'
...
export default function App() {
  ...
  return (
    <main className="min-h-screen bg-[#030407] text-white selection:bg-[#4f7dff] selection:text-white">
      <nav className="fixed inset-x-0 top-0 z-[100] border-b border-white/10 bg-[#030407]/75 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-8">
          <a href="#top" className="flex items-center gap-3 font-semibold tracking-tight"><span className="grid size-9 rotate-45 place-items-center border border-[#5f86ff]/55 bg-[#4f7dff]/10 shadow-[0_0_28px_rgba(79,125,255,.18)]"><span className="-rotate-45 text-xs">D</span></span><span>Diamond Design System</span></a>
          ...
          <a href="#pricing" className="rounded-full bg-[#4f7dff] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_0_30px_rgba(79,125,255,.35)] transition hover:scale-[1.03] hover:bg-[#658dff]">Get access</a>
        </div>
      </nav>

      <section id="top" className="relative pt-20">
        <div className="pointer-events-none absolute inset-x-0 top-20 z-30 mx-auto max-w-7xl px-6 pt-14 lg:px-8 lg:pt-20">
          <div className="max-w-[430px]">
            <p className="mb-5 text-xs font-medium uppercase tracking-[0.30em] text-[#6f93ff]">Blender plugin for jewelry designers</p>
            <h1 className="text-4xl font-semibold leading-[0.95] tracking-[-0.05em] sm:text-5xl lg:text-[4.35rem]">Design extraordinary jewelry faster.</h1>
          </div>
        </div>
        <BrokenByDesign title="Diamond Design System" height="min(900px, 92dvh)" className="min-h-[720px]" />
        <div className="pointer-events-none absolute inset-x-0 bottom-10 z-30 mx-auto flex max-w-7xl items-end justify-between px-6 lg:px-8">
          <p className="max-w-md text-sm leading-6 text-white/55 sm:text-base">Diamond Design System gives jewelry artists a premium workflow for faster modeling, cleaner results and production-ready design.</p>
          <a href="#pricing" className="pointer-events-auto hidden items-center gap-2 rounded-full border border-[#7395ff]/40 bg-[#4f7dff]/15 px-5 py-3 text-sm text-white backdrop-blur-md shadow-[0_0_26px_rgba(79,125,255,.16)] transition hover:bg-[#4f7dff] sm:flex">Join for $50/month <ArrowRight size={16} /></a>
        </div>
      </section>
